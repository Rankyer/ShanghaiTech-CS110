...
int step = 500; // Number of update steps
#define EPS 1e-5
...
double calculate_error(int N, double *ref_p, double *ref_p_next, double *p) {
  double diff = 0;
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < N; j++) {
      if (fabs(ref_p[i * N + j] - p[i * N + j]) <
...
int main(void) {
  ...
  display_time(start, end);
  printf("end\n");
  double error = calculate_error(N, ref_p, ref_p_next, p);
